...
const Header = () => {
  ...
  const dropdownLinkStyle = "block w-full px-4 py-2 text-left text-sm text-gray-700 hover:text-blue-600";
  const mobileLinkStyle = "block py-2 px-4 text-lg text-gray-700 font-semibold hover:bg-blue-50 hover:text-blue-600 rounded-md";
  const mobileSubLinkStyle = "block py-2 pl-8 pr-4 text-gray-600 hover:bg-gray-100 hover:text-blue-600 rounded-md"
    ...
    <header className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50">
      <div className="container mx-auto flex justify-between items-center py-5 px-6">
        ...
        <nav className='hidden md:flex'>
          <ul className='flex space-x-6 items-center'>
            <li>
              <NavLink
              to="/"
              className={({isActive}) => `font-semibold text-gray-600 pb-1 ${isActive ? 'text-blue-600 border-b-2 border-blue-600' : 'hover:text-blue-600'}`}
              >
                Inicio
              </NavLink>
            </li>
            ...
            <li className='relative group'>
              <button className='font-semibold text-gray-600 pb-1 hover:text-blue-600 flex items-center' aria-haspopup="true">
                Análisis de CNN
                <svg className='w-4 h-4 ml-1' fill='none' stroke='currentColor' viewBox='0 0 24 24'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth="2" d='M19 9l-7 7-7-7'></path></svg>
              </button>

              <div className='absolute left-0 pt-2 w-64 bg-white rounded-md shadow-lg z-50 hidden group-hover:block'>
                <ul className='py-1'>
            ...
            <li>
              <NavLink
              to="/contrapunto"
              className={({isActive}) => `font-semibold text-gray-600 pb-1 ${isActive ? 'text-blue-600' : 'hover:text-blue-600'}`}
              >
                Contrapunto (Olé)
              </NavLink>
            </li>
            <li>
              <NavLink to="/entrevista" 
                className={({isActive}) => `font-semibold text-gray-600 pb-1 ${isActive ? 'text-blue-600 border-b-2 border-blue-600' : 'hover:text-blue-600'}`}
              >
                Entrevista
              </NavLink>
        ...
        <div className='md:hidden'>
          <button
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} aria-label='Abrir menú'
          >
            {isMobileMenuOpen ? (
      ...
      {isMobileMenuOpen && (
        <nav className='md:hidden bg-white border-t border-gray-200 shadow-md'>
          <ul className='flex flex-col p-4 space-y-1'>
            <li>
              <NavLink to="/" className={mobileLinkStyle} onClick={closeMobileMenu}>Inicio</NavLink>
            </li>
            <li>
              <span className="block py-2 px-4 text-lg text-gray-400 font-bold">Análisis (CNN)</span>
            </li>
            ...
            <li>
              <NavLink to="/contrapunto" className={mobileLinkStyle} onClick={closeMobileMenu}>
                Contrapunto (Olé)
              </NavLink>
            </li>
            <li>
              <NavLink to="/entrevista" className={mobileLinkStyle} onClick={closeMobileMenu}>
                Entrevista
              </NavLink>
            </li>
            <li>
              <NavLink to="/staff" className={mobileLinkStyle} onClick={closeMobileMenu}>
                Staff
              </NavLink>
            </li>
          </ul>
